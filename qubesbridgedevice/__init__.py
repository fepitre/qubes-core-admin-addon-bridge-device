...
class BridgeDeviceExtension(qubes.ext.Extension):
    ...
    def on_domain_spawn(self, vm, event, start_guid, **kwargs):
        for bridge in vm.devices["bridge"].assignments():
            self.on_device_pre_attach_bridge(
                vm, event, bridge.device, bridge.options
            )

            self.on_device_attach_bridge(
                vm, event, bridge.device, bridge.options
            )
    ...
    def on_domain_pre_shutdown(self, vm, event, **kwargs):
        attached_vms = [
            domain for domain in self.attached_vms(vm) if vm.is_running()
        ]
    ...
    def generate_unused_mac(vm):
        ...
        used = [
            target.get("dev", None)
            for target in parsed_xml.xpath("//domain/devices/interface/mac")
        ]
    ...
    def create_qdb_entries(vm, options):
        ...
        if "ip" in options and "netmask" in options:
            vm.untrusted_qdb.write(
                "/net-config/" + options["mac"] + "/ip", options["ip"]
            )
            vm.untrusted_qdb.write(
                "/net-config/" + options["mac"] + "/netmask", options["netmask"]
            )
